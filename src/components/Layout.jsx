...
const Navbar = () => {
    return (
        <nav className="fixed top-0 left-0 w-full z-40 px-6 py-6 mix-blend-difference flex justify-between items-center">
            <Link to="/" className="font-sans font-bold text-xl tracking-tighter text-background hover:text-accent transition-colors">
                TENEX
            </Link>
...
const Footer = () => {
    return (
        <footer className="relative bg-dark pt-32 pb-12 px-6 md:px-12 border-t border-accent/20 overflow-hidden mt-24">
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-full h-96 pointer-events-none" style={{ background: 'radial-gradient(ellipse 70% 60% at 50% 100%, rgba(232,80,26,0.22) 0%, transparent 70%)' }}></div>
            <div className="max-w-6xl mx-auto flex flex-col gap-24 relative z-10">
                <div className="flex flex-col items-start gap-8">
                    ...
                    <button className="magnetic flex items-center gap-4 bg-accent text-primary px-8 py-4 rounded-full font-sans font-bold text-lg hover:bg-glow transition-colors duration-500 mt-8">
                        Initiate Contact
